...
class Admincp extends MX_Controller {
	...
	function index(){
      header('Location: '.PATH_URL_ADMIN.'dashboard');
      exit();
		// $data['module'] = 'admincp';
	...
	function login(){
		if(!empty($_POST)){
            ...
            $user_info = $this->model->checkLogin($this->input->post('user'));
				if($user_info && md5($this->input->post('pass'))==$user_info->password){
               $this->session->set_userdata('userInfo', $user_info->username);
	...
	function chk_perm($id_module,$type,$isAjax){
      ...
      // pr($pos,1);
      // $sub_str = substr($info[0]->permission,$pos,6);
	...
	function update_profile(){
		if(!empty($_POST)){
         $userInfo = $this->model->checkLogin($this->session->userdata('userInfo'));
			if(md5($this->input->post('oldpassAdmincp'))==$userInfo->password){
				$data = array(
				...
				if($this->db->update('admin_nqt_users',$data)){
					$this->load->model('admincp_accounts/admincp_accounts_model');
               $userInfoNew = $this->admincp_accounts_model->getData($this->session->userdata('userInfo'));
					$this->saveLog('update_profile',$userInfoNew[0]->id,'password','Update',$this->input->post('oldpassAdmincp'),$this->input->post('newpassAdmincp'));
