							...
							<table class="table table-striped table-bordered table-hover dataTable no-footer">
								<thead>
									<tr role="row">
										...
										<th class="sorting" onclick="sort('name')" id="name">Tên danh mục</th>
										<th class="center sorting" onclick="sort('parent_id')" id="parent_id" width="200">Danh mục cha</th>
										<th class="center sorting" onclick="sort('category_type_id')" id="category_type_id" width="200">Loại sản phẩm</th>
								...
								<tbody>
									...
									<tr class="item_row<?=$i?> gradeX <?php ($k%2==0) ? print 'odd' : print 'even' ?>" role="row">
										...
										<td class="center"><?=$v->parent_name?></td>
										<td class="center"><?=$v->product_type?></td>
										<td class="center" id="loadStatusID_<?=$v->id?>"><a class="no_underline" href="javascript:void(0)" onclick="updateStatus(<?=$v->id?>,<?=$v->status?>,'<?=$module?>')"><?php ($v->status==0) ? print '<span class="label label-sm label-default">Khóa</span>' : print '<span class="label label-sm label-success">Mở</span>' ?></a></td>
