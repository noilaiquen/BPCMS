...
$(document).ready(function() {
   ...
   var url = $.url(document.location.href);
	if(url.segment(-1)!='update' && url.segment(-2)!='update' && url.segment(-1)!='import_store' && url.segment(-1)!='import_plan' && url.segment(-1)!='dashboard'){
...
function show_perm_denied(message = undefined){
   clearTimeout(timeoutNotification);
   if (message) {
      $('#txt_error').html(message);
   }
	$('.notification').show();
...
function show_perm_success(message = undefined){
	clearTimeout(timeoutNotification);
	$('.notification').show();
		...
		if(document.getElementById('item'+i)!=null){
			if(document.getElementById('item'+i).checked==true){
				...
				$.post(url,{
                  ...
                  $('#txt_error').html('Can not deleted category because this category link to other categories!');
						show_perm_denied();
						return false;
               } else{
...
function show_loading(){
	...
	$('.loading_content .loading_text').html('Đang load dữ liệu');
...
function hide_loading(){
	setTimeout(function(){
		...
		// $('.loading_content .loading_text').hide();
	},500);
}